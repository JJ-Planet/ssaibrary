...
function Floor() {
  ...
  const toDetail = (roomId) => {
    ...
    movePage(`/SelfStudyRoomPage/${roomId}`);
  };
  const nothing = () => {
  ...
  if (isLoadingStudyRoom && isLoadingRoom) {
    return (
      <>
        ...
        >
          <span className={styles.Room1Num}>{studyRooms[0].id}</span>
          <div className={styles.BigRoomStatus}>
            {studyRooms[0].status == "W" ? "사용 중" : "예약 가능"}
          </div>
          <div className={studyRooms[0].status == "W" ? styles.Full : styles.Empty}></div>
        </div>
        ...
        >
          <span className={styles.Room2Num}>{studyRooms[1].id}</span>
          <div className={styles.BigRoomStatus}>
            {studyRooms[1].status == "W" ? "사용 중" : "예약 가능"}
          </div>
          <div className={studyRooms[1].status == "W" ? styles.Full : styles.Empty}></div>
        </div>
        ...
        >
          ...
          <div className={styles.SmallRoomStatus}>
            {rooms[0].isAvailable == "N"
              ? "예약 불가"
              : `${rooms[0].reserveSeat} / ${rooms[0].totalSeat}`}
          </div>
        ...
        >
          ...
          <div className={styles.SmallRoomStatus}>
            {rooms[1].isAvailable == "N"
              ? "예약 불가"
              : `${rooms[1].reserveSeat} / ${rooms[1].totalSeat}`}
          </div>
        ...
        >
          ...
          <div className={styles.SmallRoomStatus}>
            {rooms[2].isAvailable == "N"
              ? "예약 불가"
              : `${rooms[2].reserveSeat} / ${rooms[2].totalSeat}`}
          </div>

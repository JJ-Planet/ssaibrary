package com.jjplanet.ssaibrary.api.member.provider;
...
public class TokenProvider implements InitializingBean {
	...
	public TokenProvider(
		...
		@Value("${app.auth.token-expiry}") long tokenValidityInSeconds,
		@Value("${app.auth.refresh-token-expiry}") long refreshTokenValidityInMilliseconds
		// RedisService redisService
	) {
		this.secret = secret;
		this.tokenValidityInMilliseconds = tokenValidityInSeconds; // 24시간
		this.refreshTokenValidityInMilliseconds = refreshTokenValidityInMilliseconds; // 2주
	...
	public Long getExpiration(TokenKey tokenKey) {
		...
		if (delimiter.equals(TokenKey.ACCESS.getKey())) {
			...
			return refreshTokenValidityInMilliseconds; // 2주
		} else {
